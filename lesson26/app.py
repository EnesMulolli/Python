...
menu = ["Dashboard", "Manage Recipes", "Manage Categoories"]
    ...
    if recipe_list:
        ...
        if manage_action == "Edit":
            ...
            if recipe_to_edit:
                ...
                if category_list:
                    ...
                    edit_category_name = st.selectbox("Category", category_name, index=category_name.index(next(cat['name'] for cat in category_list
                                                                                                                if cat['id'] == selected_recipe['category_id'])), key="edit_recipe_category")
                ...
                if st.button("Update Recipe", key="update_recipe_button"):
                    if all([edit_name, edit_description,  edit_ingredients, edit_instructions, edit_cuisine, edit_difficulty, edit_category_id is not None]):
                        ...
                        st.success(f"Recipe '{edit_name}' was updated successfully!")
                    else:
                        st.error("All fields must be filled and category selected to update recipe.")
            ...
            if recipe_to_delete:
                ...
                if st.button(f"Delete '{selected_recipe['na,e']}'", key="delete_recipe_button"):
                    delete_recipe(selected_recipe['id'])
                    st.success(f"Recipe '{selected_recipe['name']}' was deleted successfully")
    else:
        st.info("No recipes availale to manage.")