...
def get_recipes(cuisine: str = None, difficulty: str = None):
    ...
    return [Recipe(id = row[0], name = row[1], description = row[2], ingredients = row[3],
               instuction = row[4], cuisine = row[5], difficulty = row[6], category_id = row[7]) for row in recipes ]
...
def create_recipe(recipe: RecipeCreate):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO recipes (name, description, ingredients, instructions, cuisine, difficulty, category_id) VALUES (?, ?, ?, ?, ?, ?, ?)", (recipe.name, recipe.description, recipe.ingredients, recipe.instructions,
