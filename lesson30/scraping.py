...
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (LHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36',
...
def extract_article(content):
    ...
    for article in soup.find_all('div', class_="search-item"):
        ...
        if title_div:
            ...
            description_tag = articles.find('p')
            if description_tag:
...
def scrap_multiple_pages(base_url, num_pages):
    all_articles = []
...
num_pages = 5
all_articles = scrap_multiple_pages(base_url, num_pages)

df = pd.DataFrame(all_articles)

df.to_csv('technewsworld_articles.csv', index=False)

print("Articles have beem saved to technewsworld_articles.csv")
